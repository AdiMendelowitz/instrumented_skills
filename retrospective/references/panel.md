...
Facilitator protocol and lens definitions for RUN mode R3. Lenses are analytical stances, deliberately kept in one file per the domain-over-persona architecture rule.
...
4. Enforce anchors and the 5-minute test at intake; return vague findings to the lens once, then drop them.
5. Enforce ELMO per thread when a discussion stops producing new root causes.
...
7. Own the synthesis handed to R4. Debate output belongs to the panel; synthesis belongs to the facilitator.
...
Standing questions: leakage, seed discipline, evaluation validity, baseline honesty? Were negative results recorded or quietly dropped? Does the pipeline hold under the project's own stated conventions (e.g. correctness before speed, from-scratch validation before optimizing)?
