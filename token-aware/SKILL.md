...
verify caching behavior empirically (`cache_read_input_tokens` after a real call) rather
...
execute later, with no shared context. Here, the output is the instructions themselves —
write them to be self-contained and correct on their own, not dependent on state only
this session has.
...
Evaluate every call site in this order. Stop at the first category that applies —
a call site gets exactly one label, from `KEEP`, `BATCH`, `REPLACE`, `DOWNGRADE`,
`CACHE`, or `TRIM`.
...
1. **BATCH** if the work is scheduled or otherwise tolerant of asynchronous processing.
   ...
   converting a synchronous call is a structural change, not a one-line edit — size it
   before committing, but it's usually the largest single lever on a scheduled workload.
2. **REPLACE** with deterministic code, if the call is doing something that doesn't
   ...
   measured agreement rate against held-out LLM-labeled examples, reported honestly even
...
5. **TRIM**, last and incremental. Reduce input by sending derived values instead of raw
   ...
   costs a multiple of input — check the current ratio rather than assuming it.
...
- `references/prompt_rules.md` — construction rules for calls authored or reviewed under
  ...
  supports it.
- `tools/cost.py` — the arithmetic. Cost, break-even, compare, estimate, plus
  `log`/`verify`/`render`/`cpd` subcommands. No network calls; refuses to compute
