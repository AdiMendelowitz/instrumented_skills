...
Standing checks: assertions that check the mock rather than the behavior, missing edge cases named in the code's own docstrings, a test suite that passes on a broken implementation because it tests the wrong layer, boundary cases stated in comments but never asserted.
...
Standing checks: the same concept named two different ways across the file, a name that implies a behavior the code doesn't have, abbreviations used without being defined once.

## Always-on additions for code targets

evidence-integrity extends to: a comment claiming a behavior the code doesn't implement, a docstring describing parameters that no longer exist.
